<?php
	...
	$LIST_SUBDEPARTMENT_ADD =  '<select style="display:none;" id="txt_subdepartmentid" name="txt_subdepartmentid">';
	$LIST_SUBDEPARTMENT_UPDATE =  '<select style="display:none;" DISABLED id="txt_subdepartmentid2" name="txt_subdepartmentid2">';
	$LIST =  '';
	$LIST .=  '<option value="0" selected>-</option>';
	
/*	
	$slno = 0;
//	if ($result1 = $mysqli->query("SELECT id, fname FROM m_subdepartment where  flag_addon = 1 AND  active = '1' order by nrank1, fname, id"))
	if ($result1 = $mysqli->query("SELECT id, fname FROM m_subdepartment where  flag_addon = 1 AND  active = '1' order by fname, id"))
	{
		if ($result1->num_rows > 0)
		{
			while ($obj1 = $result1->fetch_object())
			{
				...
				$pcategoryname = $obj1->fname;
				$LIST .=  '<option value="' . $subdepartmentid2 . '">' . $pcategoryname .'</option>';

				$ARR_SUBDEPARTMENT[$subdepartmentid2] = $pcategoryname;
			}
			$result1->close();
		} 
		else
	...
	*/ 
	...
	$LIST_SUBDEPARTMENT_ADD .=  $LIST;
	$LIST_SUBDEPARTMENT_UPDATE .=  $LIST;
	$LIST =  '';
		...
		<div class="w3-card-4">
			...
			<div class="w3-container">
				<form id="AddForm">
					<table class="table table-striped table-bordered table-hover">
						...
						<tr><td><?php echo $LIST_SUBDEPARTMENT_ADD; ?></td></tr>
			...
			<footer class="w3-container w3-green">
				<p class="w3-rest w3-right-align">
					<button type="button" class="w3-button w3-indigo w3-hover-blue" onclick="fn_addon_add('ADD');">Save</button>
				</p>
			</footer>
		</div> 		  
      ...
      <p>
		...
		<div class="w3-card-4">
			...
			<div class="w3-container">
				<form id="UpdateForm">
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<tr><td><?php echo $LIST_SUBDEPARTMENT_UPDATE; ?></td></tr>
			...
			<footer class="w3-container w3-blue">
				<p class="w3-rest w3-right-align">
					<button type="button" class="w3-button w3-indigo w3-hover-blue" onclick="fn_addon_update('UPDATE');">Save</button>
				</p>
			</footer>
		</div> 		  
...
<a href="m_subdepartment_list.php" class="w3-button w3-blue">If sub department not linked, kindly tick the Addon flag</a>	
...
function fn_addon_edit(recid)
{
	...
	document.getElementById('txt_subdepartmentid2').value = document.getElementById('txt_pcategory_' + recid).value;
...
function fn_addon_update() 
{
	...
	data.append("subdepartmentid", document.getElementById("txt_subdepartmentid2").value);
...
function fn_addon_mark(arg_id, arg_field, arg_flag)
{
	var data = new FormData();
	data.append("opt", 3);
...
function fn_addon_delete(arg_id, arg_tstamp)
{
	var data = new FormData();
	data.append("opt", 9);
	data.append("rid", arg_id);
